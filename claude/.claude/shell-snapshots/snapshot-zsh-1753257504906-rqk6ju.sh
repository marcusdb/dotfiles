...
command_not_found_handler () {
	if [[ "$1" != "mise" && "$1" != "mise-"* ]] && /Users/marcusdb/.local/bin/mise hook-not-found -s zsh -- "$1"
	then
		_mise_hook
		"$@"
...
kube_ps1 () {
	...
	if [[ -z "${KUBE_PS1_PREFIX_COLOR:-}" ]] && [[ -n "${KUBE_PS1_PREFIX}" ]]
	then
	...
	if [[ -n "${KUBE_PS1_SEPARATOR}" ]] && [[ "${KUBE_PS1_SYMBOL_ENABLE}" == true ]]
	then
	...
	then
		if [[ -n "${KUBE_PS1_DIVIDER}" ]] && [[ "${KUBE_PS1_CONTEXT_ENABLE}" == true ]]
		then
			KUBE_PS1+="${KUBE_PS1_DIVIDER}" 
		fi
		KUBE_PS1+="$(_kube_ps1_color_fg ${KUBE_PS1_NS_COLOR})${KUBE_PS1_NAMESPACE}${KUBE_PS1_RESET_COLOR}" 
	fi
	if [[ -z "${KUBE_PS1_SUFFIX_COLOR:-}" ]] && [[ -n "${KUBE_PS1_SUFFIX}" ]]
	then
...
mise () {
	...
	if [ "$#" = 0 ]
	then
		command /Users/marcusdb/.local/bin/mise
		return
	fi
	shift
	case "$command" in
		(deactivate | shell | sh) if [[ ! " $@ " =~ " --help " ]] && [[ ! " $@ " =~ " -h " ]]
			then
				eval "$(command /Users/marcusdb/.local/bin/mise "$command" "$@")"
				return $?
			fi ;;
	esac
	command /Users/marcusdb/.local/bin/mise "$command" "$@"
}
mkcd () {
	mkdir -p $@ && cd ${@:$#}
}
...
export PATH=/opt/homebrew/opt/node\@22/bin\:/Users/marcusdb/.local/share/mise/installs/direnv/2.35.0\:/opt/homebrew/opt/openjdk\@17/bin\:/Users/marcusdb/.groundcover/bin\:/Users/marcusdb/.rd/bin\:/opt/homebrew/bin\:/opt/homebrew/sbin\:/usr/local/bin\:/System/Cryptexes/App/usr/bin\:/usr/bin\:/bin\:/usr/sbin\:/sbin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin\:/Library/Apple/usr/bin\:/usr/local/MacGPG2/bin\:/Applications/iTerm.app/Contents/Resources/utilities\:/Users/marcusdb/.orbstack/bin\:/Users/marcusdb/Library/Android/sdk/platform-tools\:/Users/marcusdb/Library/Android/sdk/tools\:/Users/marcusdb/Library/Android/sdk/tools/bin\:/Users/marcusdb/Library/Android/sdk/emulator\:/Users/marcusdb/.local/bin
